...
class NewMAnet(nn.Module):
    ''' Network of MANet'''
    def __init__(self, in_nc=3, kernel_size=21, nc=[128, 256], nb=1, split=2):
        super(NewMAnet, self).__init__()
        self.kernel_size = kernel_size
...
class MANet_s1(nn.Module):
    ...
    def __init__(self, in_nc=3, out_nc=3, nf=64, nb=10, gc=32, scale=4, pca_path='./pca_matrix_aniso21_15_x2.pth',
        ...
        # self.kernel_estimation = NewMAnet(in_nc=in_nc, kernel_size=kernel_size, nc=[manet_nf, manet_nf * 2],
                                        #  nb=manet_nb, split=split)
    ...
    def forward(self, x, gt_K):
        ...
        # print(kernel.size())
        # kernel = F.interpolate(kernel, scale_factor=self.scale, mode='nearest')
        # print(kernel.size())

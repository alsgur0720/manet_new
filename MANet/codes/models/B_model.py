...
from models.loss import Cus_Loss
import sys
...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        self.custom_loss = Cus_Loss(opt).to(self.device)    
    ...
    def optimize_parameters__hiheon(self, step, scaler):
        self.optimizer_G.zero_grad()
        l_all = 0
        self.fake_SR, params = self.netG(self.var_LN, self.real_K)
        param_loss, self.fake_K = self.custom_loss(params, self.ker_map, self.real_K)
        # print(self.fake_K)
        # l_all += ken_loss
        l_all += param_loss
        # self.log_dict['l_ker'] = ken_loss.item()
        self.log_dict['l_par'] = param_loss.item()
        
        # hr loss
        if self.cri_pix is not None:
            l_pix = self.l_pix_w * self.cri_pix(self.fake_SR, self.real_H)
            l_pix.requires_grad = True
            l_all += l_pix
        ...
        # if self.cri_ker is not None:
            ...
            # l_ker = self.l_ker_w * self.cri_ker(self.fake_K * 10000, self.real_K * 10000) / b
            # l_ker = self.l_ker_w * self.cri_ker(params, self.ker_map) / b
            # print(params, self.ker_map)
            # l_all += l_ker
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            # param_loss, fake_K = self.custom_loss(params, self.ker_map, self.real_K)

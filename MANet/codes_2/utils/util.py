...
def anisotropic_gaussian_kernel_tensor(l, sig1, sig2, theta, tensor=False):
    # mean = [0, 0]
    # v = np.dot(np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta), np.cos(theta)]]), np.array([1., 0.]))
    # V = np.array([[v[0], v[1]], [v[1], -v[0]]]) # [[cos, sin], [sin, -cos]]
    # D = np.array([[sig1, 0], [0, sig2]])
    ...
    # x, y = np.mgrid[-center:-center+l:1, -center:-center+l:1]
    x = torch.arange(-center,-center+l,1,dtype=torch.float32)
    y = torch.arange(-center,-center+l,1,dtype=torch.float32)
    pos = torch.dstack((y, x))
...
def kernel_maker(batch, l=21, sig=2.6, sig1=2.6, sig2=2.6, theta=0, rate_iso=1.0, scale=3, tensor=True):
    batch_kernel = np.zeros((batch, l, l))
    batch_sigma = np.zeros((batch, 3))
    shifted_l = l - scale + 1
    for i in range(batch):
